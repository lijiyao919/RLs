...
def train():
    ...
    for i_episode in count(1):
        ...
        while True:
            ...
            residual_state = residual_state.astype(np.float32).ravel()                                   #flattern the 80*80 image
            residual_state_tensor = T.from_numpy(np.expand_dims(residual_state, axis=0)).to(device)      #change numpy to tensor
            action, log_prob = agent.select_action(residual_state_tensor)
            if action == 0:
                state, reward, done, _ = env.step(UP)
            else:
                state, reward, done, _ = env.step(DOWN)
            #env.render()
            agent.collect_experience(residual_state, action, log_prob, reward)
            ep_reward += reward
